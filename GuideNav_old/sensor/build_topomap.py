...
def extract_keyframes(input_dir: str, output_dir: str, 
    ...
    Args:
        input_dir: Input directory containing /color, odom.csv, and optionally /depth
            (depth is only used opportunistically for output alignment; keyframe
            selection itself is purely odometry-based and works without it)
    ...
    for dir_path in [rgb_output_dir, topo_output_dir]:
    ...
    has_depth = len(depth_timestamps) > 0
    if has_depth:
        depth_output_dir.mkdir(parents=True, exist_ok=True)
    else:
        print("No depth images found - proceeding without depth "
              "(keyframe selection only uses odometry, depth is not required).")

    print(f"Found {len(rgb_timestamps)} RGB images and {len(depth_timestamps)} depth images")

    # Find aligned timestamps (RGB images that have corresponding odometry;
    # depth is attached opportunistically when available, but never required)
    aligned_data = []

    for rgb_timestamp_str in rgb_timestamps:
        rgb_timestamp = float(rgb_timestamp_str)

        # Find closest depth timestamp (optional)
        depth_timestamp_str = (
            find_closest_timestamp(rgb_timestamp, depth_timestamps, max_time_diff)
    ...
    for i, keyframe in enumerate(keyframes):
        ...
        # Copy depth image (optional - keyframe is still kept if depth is missing)
