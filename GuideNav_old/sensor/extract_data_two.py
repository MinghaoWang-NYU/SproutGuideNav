...
建图走 naive 方式 (sensor/build_topomap.py) 时用 color/ + odom.csv 做时间戳对齐, depth/
...
存在几百秒量级的固定偏移 (两个子系统时钟没同步), 导致 build_topomap.py 用 header.stamp
对齐 RGB/odom 时间戳时永远匹配不上、产出 0 个关键帧。改用统一的本地接收时钟后, 三路数据
在同一个进程里打点, 天然对齐, 不依赖机器人底层各子系统的时钟同步。
"""
import rclpy
...
class SimpleImageSaver(Node):
    ...
    def _now_str(self):
        """本地接收时钟的时间戳字符串, 三路数据统一用这个而不是各自的 msg.header.stamp。"""
        now = self.get_clock().now().seconds_nanoseconds()
        return f"{now[0] + now[1] * 1e-9:.9f}"

    def zed_rgb_callback(self, msg):
        try:
            ts_str = self._now_str()
    ...
    def status_callback(self):
        ...
        if self.depth_count == 0:
            self.get_logger().warning(
                ...
                "naive 建图 (build_topomap.py) 现已支持 depth 缺失的情况, 此警告可忽略")
